...
if(fork() == 0){ // fork to create child process and run our shell from there so if it breaks it will be catched by wait()
  ...
  return 0; // if everythign fails, it exits by its return 0; if code touches here somehow breaking while loop up there; return is used to break function like break; for loops,
} //if return not given still compiles but compiler complains specially the when the return type is intiger. If its void then you can write return; or you can skip it but in int you gotta return a intiger...
// i guess i tryed to explan as much as i knew; whoever is reading and reached the end congyratulasitution If liked it then don't forget to leave a star or do something in repo that helps/appreciates...
