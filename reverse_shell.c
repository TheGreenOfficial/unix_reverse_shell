...
if(fork() == 0){ // fork to create child process and run our shell from there so if it breaks it will be catched by wait()
  ...
  return 0; // if everythign fails, it exits by its return 0; if code touches here somehow breaking while loop up there; return is used to break function like break; for loops if this not given still compiles but compiler complains specially the return type is intiger if its void then you can write return; or you can leave it like that but in int you gotta return a intiger...
}
